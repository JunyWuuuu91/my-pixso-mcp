...
function repoRootFromDistEntry(): string {
...
function printCodexConfig(flags: Record<string, string | boolean>): void {
  const config = buildConfig(flags);
  const entryPath = currentEntryPath();
  console.log(renderCodexConfigSnippet(config, entryPath));
}

function readFlagString(flags: Record<string, string | boolean>, name: string, fallback: string): string {
  const value = flags[name];
  return typeof value === 'string' && value.trim() ? value.trim() : fallback;
}

function printQuickstart(flags: Record<string, string | boolean>): void {
  const config = buildConfig(flags);
...
function printInstallCodexConfigResult(flags: Record<string, string | boolean>): void {
  const config = buildConfig(flags);
  const configPath = readFlagString(flags, 'config', defaultCodexConfigPath());
...
${result.block}`);
  ...
  if (result.backupPath) console.log(`Backup created: ${result.backupPath}`);
  if (!result.updated) console.log('The managed pixso_advanced block was already up to date.');
  console.log(`Transport: ${config.transport}`);
}

async function runServe(flags: Record<string, string | boolean>): Promise<void> {
  const config = buildConfig(flags);
...
function renderConfigPreview(configPath: string): string {
  try {
