...
export function renderCodexConfigSnippet(config, entryPath) {
...
export function renderManagedCodexConfigBlock(config, entryPath) {
    return `${BEGIN_MARKER}
${renderCodexConfigSnippet(config, entryPath).trimEnd()}
${END_MARKER}
`;
}
export function hasPixsoCodexConfig(text) {
    return text.includes('[mcp_servers.pixso_advanced]');
}
export function upsertManagedCodexConfig(existingText, block) {
...
function backupPathFor(configPath, backupSuffix) {
    const suffix = backupSuffix ?? new Date().toISOString().replace(/[:.]/g, '-');
    return `${configPath}.bak-${suffix}`;
}
export function installCodexConfig(options) {
